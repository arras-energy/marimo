...
def __(
    ...
    set_mapview(load_map(data["objects"]))
    mo.ui.tabs(
        {
            "Table": mo.lazy(
                mo.ui.table(
...
def __(columns, data, mo, pd, px):
    ...
    satellite_ui = mo.ui.switch(
        label="Satellite view", value=get_satellite(), on_change=change_satellite
    )
    label_ui = mo.ui.switch(
        label="Show labels", value=get_labels(), on_change=change_labels
    )
    ...
    def load_map(objects, oclass=None):
        ...
        map.update_layout(margin={"r": 0, "t": 0, "l": 0, "b": 0}, dragmode="pan")
        set_mapview(map)
        return mo.vstack(
            [map, mo.hstack([satellite_ui, label_ui], justify="center")]
        )
    return (
        change_labels,
        change_satellite,
...
def __(mo):
    ...
    $^1$ SLAC National Accelerator Laboratory is operated by Stanford University for the US Department of Energy (DOE) under Contract DE-AC02-67SF00515. This work was funded by the US DOE Office of Electricity Advanced Grid Modeling Program and a grant from the Grid Controls and Communications Division.
...
async def __():
    ...
    return go, json, micropip, mo, os, pd, pio, px, re, sys
